...
export default class LwcModal extends LightningElement {
    ...
    /* PUBLIC PROPERTIES */
    _showModal; // Boolean to show/hide the modal
    _focusIndex = 0;    // Index of the currently focused element, ranging from 0 to the number of input elements

    /* Modal properties */
    _header;    // Modal header text
    _bodyText;  // Modal body text
    _cancelButtonLabel = DEFAULTS.CANCEL_BUTTON_LABEL;   // Label of the Cancel button
    _confirmButtonLabel;    // Label of the Confirm button
    _confirmButtonVariant = DEFAULTS.CONFIRM_BUTTON_VARIANT;  // Label of the Confirm button
    ...
    _focusSelectorString; // selector string to identify the element to focus when the modal opens.
    @api errorMessage; // Message to display when the modal is not valid. Defaults to 'Your browser is outdated. Your Salesforce experience may be degraded.' if not set.
    ...
    set confirmButtonLabel(value) {
        this._confirmButtonLabel = value;
    }

    @api
    get confirmButtonVariant() {
        return this._confirmButtonVariant;
    }
    set confirmButtonVariant(value) {
        this._confirmButtonVariant = value;
    }
    ...
    @api
    get focusSelectorString() {
        return this._focusSelectorString;
    }
    set focusSelectorString(value) {
        this._focusSelectorString = value;
    }
    ...
    set confirmation(confirmation) {
        ...
        if (confirmation) {
            ...
            this._header = confirmation.header;
            this._bodyText = confirmation.text;
            this._confirmButtonLabel = confirmation.confirmButtonLabel || DEFAULTS.CONFIRM_BUTTON_LABEL;
            this._confirmButtonVariant = confirmation.confirmButtonVariant || DEFAULTS.CONFIRM_BUTTON_VARIANT;
            this._cancelButtonLabel = confirmation.cancelButtonLabel || DEFAULTS.CANCEL_BUTTON_LABEL;
            // Open the modal
            if (!this.showModal)
                this.open();
        } else {
    ...
    renderedCallback() {
        ...
        //} else {
            if (this.focusSelectorString) {
                ...
                this._focusSelectorString = null;
            }
        }
    }
    ...
    handleElementFocus = (event) => {
        this._focusIndex = event.currentTarget.dataset.focusIndex;
    }
    ...
    handleButtonClick(event) {
        ...
        // Dispatch the buttonclick event
